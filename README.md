...
- session당 후보 광고 수 분포가 매끄러운 종형이 아니라 4개와 6개 지점에서 두 번 봉우리가 생기는 이봉분포로 나타남. 평균(5.16)만 봐서는 드러나지 않던 패턴으로, 세션 구성 방식 자체에 후보군 크기를 나누는 규칙이 있을 가능성을 시사함
- category의 confidence_level 분포는 0 근처 외에 0.9~1.0 구간에서도 뚜렷한 두 번째 봉우리를 보임. topic은 이런 패턴이 없이 낮은 신뢰도에 집중됨. 문서마다 확실한 주 카테고리가 하나씩 존재한다는 의미이며, 베이스라인에서 primary_category 하나만 뽑아 쓴 이유와도 연결됨
